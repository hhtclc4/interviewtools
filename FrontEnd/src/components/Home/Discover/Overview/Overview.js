...
class DiscoverOverview extends React.Component {
  ...
  render() {
    let { data, image_index } = this.props;
    ...
    return (
      <div className="do-container d-flex flex-row">
        ...
        >
          <div className="do-recruit-title">{data.title}</div>
          <div className="do-recruit-company">{data.user.company.name}</div>
          <div className="do-recruit-detail d-flex flex-row justify-content-between flex-fill align-items-center">
            <div className="do-salary">
              ...
              {data.salary !== 0 ? `${data.salary}$` : "Lương thỏa thuận"}
            </div>
...
export default withRouter(DiscoverOverview);
